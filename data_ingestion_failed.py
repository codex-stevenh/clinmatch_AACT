    ...
    try:
        ...
        for nct_id in tqdm(failed_ids):
            ...
            try:
                ...
                # create multiple payloads with merged_text splitted into 4000 characters with overlap of 100 characters
                ...
                for i, payload in enumerate(payloads):
                    ...
                    if lambda_response_len == 0:
                        lambda_results_all_payloads = []
                        with open(failed_log_dir, 'a') as log_file:
                            log_file.write(f"{nct_id}\n")
                        # skipping this record
                        logger.info(f"Error processing record with nct_id {record['nct_id']} - Lambda null output")
                        break
                    else:
                        logger.info(f"Lambda response cnt - segement {i}: {lambda_response_len}")

                        for metamap_result in lambda_results: 
                            metamap_result['onco_tag'] = metamap_result['cui'] in cui_df_cancer_snomed_ct_tree
                        
                        lambda_results_all_payloads.extend(lambda_results)

                
                if len(lambda_results_all_payloads) != 0:
                    ...
                    item = {
                        'nct_id': nct_id,
                        'updated_at': updated_at_str,
                        'metamap_result': lambda_results_all_payloads
                    }

                    # write to local json file with nct_id as filename
                    with open(f'{phase_dir}/{nct_id}.json', 'w') as f:
                        json.dump(item, f, indent=4)
                
                # Step 8: Write to DynamoDB        
                # write_to_dynamodb(nct_id, updated_at, lambda_results_all_payloads)
